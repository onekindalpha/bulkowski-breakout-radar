...
def load_metadata(path_or_file=None) -> pd.DataFrame:
    frames = []
    seed = KR_DIR / "ticker_master_korea_seed.csv"
    if seed.exists():
        frames.append(pd.read_csv(seed))
    full_master = KR_DIR / "ticker_master_korea.csv"
...
with st.sidebar:
    ...
    market = st.radio("Market", ["Korea"], horizontal=True)
    st.caption("기본값은 data/kr 폴더의 최신 CSV를 읽는다. 다른 결과를 보려면 아래에서 업로드하면 된다.")

    report_file = st.file_uploader("report_v2.csv", type=["csv"])
    premarket_file = st.file_uploader("premarket_auto_korea.csv", type=["csv"])
    overlay_file = st.file_uploader("thesis_overlay_master.csv", type=["csv"])
    meta_file = st.file_uploader("ticker_master_korea.csv (optional)", type=["csv"])
    ...
    live_fill_names = st.checkbox("Auto-fill visible Korean names", value=True, help="표에 보이는 일부 숫자 이름을 Naver에서 즉시 조회한다. 많으면 약간 느릴 수 있다.")

report_source = report_file if report_file is not None else default_path("report_v2.csv")
premarket_source = premarket_file if premarket_file is not None else default_path("premarket_auto_korea.csv")
...
with st.sidebar:
    ...
    st.caption("종목명이 숫자로 보이면 아래 버튼을 누른다. v5는 pykrx 기본 종목명 → ETF명 → Naver Finance 순서로 강제 재조회한다.")
    if st.button("Build / refresh Korean names", use_container_width=True):
        if report_file is not None:
            st.warning("업로드 파일 모드에서는 먼저 CSV를 data/kr에 저장한 뒤 실행하는 편이 안전하다.")
        else:
            script = Path(__file__).with_name("build_ticker_master_korea.py")
            out_path = KR_DIR / "ticker_master_korea.csv"
            tickers_path = default_path("tickers_korea.txt")
            ...
            with st.spinner("종목명/업종명 마스터 생성 중... pykrx가 실패하면 Naver 이름 fallback으로 진행한다."):
                try:
...
with tab3:
    ...
    template["industry"] = ""
    st.download_button(
        "Download ticker_master_korea template",
        template.to_csv(index=False).encode("utf-8-sig"),
        file_name="ticker_master_korea_template.csv",
        mime="text/csv",
    )
...
with tab4:
    ...
    st.download_button(
        ...
        file_name="bulkowski_breakout_radar_korea_processed.csv",
        mime="text/csv",
    )
