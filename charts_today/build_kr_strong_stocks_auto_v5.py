...
Patched strong-stocks builder:
- includes kr_foreign_naver_auto.txt as a seed if present
...
def fetch_history(sym: str) -> pd.DataFrame:
    tk = yf.Ticker(sym)
    return tk.history(period="3mo", interval="1d", auto_adjust=False)
...
def main():
    ...
    for i, sym in enumerate(tickers, 1):
        ...
        if i in (20, 40, 60, total):
            print(f"... strong stocks {i}/{total}")

    pd.DataFrame(debug).to_csv("kr_strong_stocks_auto_debug.csv", index=False)
