...
# -*- coding: utf-8 -*-
"""
build_kr_foreign_naver_auto_v3.py

Anchor-based daily builder for Korea foreign net-buy overlay from Naver Finance.
...
Strategy:
1) Fetch main Naver ranking page
2) If page contains iframes, also inspect same-domain iframe pages
3) Find tables/blocks with stock anchors (item/main.naver?code=xxxxxx)
4) Prefer blocks whose surrounding text mentions 외국인 and 순매수
5) Parse rows directly from anchor-containing <tr> elements
6) Resolve .KS/.KQ suffix using pykrx, fallback to yfinance probe
"""
...
def extract_rows_from_table(table, page_url: str):
    ...
    for tr in table.find_all("tr"):
        ...
        qty = None
        if nums:
            # Heuristic: first/second numeric columns often qty/amount or amount/qty.
            # Keep both; ranking order on page matters more than exact value.
            amount = nums[0]
...
def collect_candidate_blocks(html: str, page_url: str):
    ...
    for i, table in enumerate(soup.find_all("table")):
        ...
        if rows:
            blocks.append({"score": score, "rows": rows, "snippet": text[:180], "page_url": page_url})

    # iframes can contain the real content on Naver pages
    iframe_urls = []
...
def pick_best_rows(blocks):
    ...
    # Prefer highest-scoring block; if tie, more parsed rows
    blocks_sorted = sorted(blocks, key=lambda b: (b["score"], len(b["rows"])), reverse=True)
    best = blocks_sorted[0]

    # If best is tiny, merge top few blocks that clearly look like same ranking family
    rows = []
    seen = set()
