#!/usr/bin/env python3
"""
update_premarket_yf_auto_debug_korea_v5.py

Includes kr_foreign_naver_auto.txt in grouped auto pricing.
Patched: dedupe fetches by ticker and include tickers_indices_fill_korea.txt.
"""
...
GROUP_FILES = [
    ...
    "tickers_indices_fill_korea.txt",
    "tickers_leverage2x_korea.txt",
    "finviz_manual_korea.txt",
]

TICKER_RE = re.compile(r"^(?:\d{6}\.(?:KS|KQ)|\d{4}\.HK|[A-Z0-9\.\-\=\^]+)$")
...
def main():
    ...
    for i, sym in enumerate(all_tickers, 1):
        ...
        if px is not None:
            prices[sym] = px
        ticker_meta[sym] = {"src": src, "ts_kst": ts.isoformat() if ts is not None else "", "error": err}
        time.sleep(0.03)
