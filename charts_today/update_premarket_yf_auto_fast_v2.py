...
# --- CLI robustness: normalize Unicode dashes to '--' (common when copy/pasting)
import sys as _sys
_UNICODE_DASHES = ['—', '–', '−']  # em dash, en dash, minus
_new_argv = []
_replaced = False
for _a in _sys.argv:
    _b = _a
    for _d in _UNICODE_DASHES:
        if _d in _b:
            _b = _b.replace(_d, '--')
            _replaced = True
    _new_argv.append(_b)
if _replaced:
    _sys.argv = _new_argv
    print('[warn] normalized unicode dashes in argv -> use standard "--" flags')
...
def main():
    ...
    ap.add_argument("--universe-file", default=None, help="Optional: ticker list file (e.g. tickers.txt). If set, scan ONLY these tickers (no group union).")
    ...
    # Universe override: if provided, ignore group union and scan ONLY those tickers
    ...
    # download once (handle empty effective)
    df = safe_download(effective) if effective else pd.DataFrame()
    ...
    for g in groups:
        for t in g.tickers:
            ...
            status = "ok" if t in got else ("skipped" if (not args.refresh_bad and t in skip) else "missing")
            rows_debug.append({
                "group": g.group,
                "ticker": t,
                "yahoo_symbol": t,
    ...
    # add age minutes (how fresh the last quote is)
    try:
        now_kr = pd.Timestamp.now(tz=ZoneInfo("Asia/Seoul")).tz_convert(None)
        ts_parsed = pd.to_datetime(debug_df.get("yahoo_ts", ""), errors="coerce")
        debug_df["age_min"] = (now_kr - ts_parsed).dt.total_seconds() / 60.0
    except Exception:
        debug_df["age_min"] = np.nan

    # quick console summary: ok but stale vs recent
    if "status" in debug_df.columns:
        ok_df = debug_df[debug_df["status"] == "ok"].copy()
        recent = ok_df[ok_df.get("age_min", 1e9) <= 15.0]
        stale = ok_df[ok_df.get("age_min", 1e9) > 15.0]
        print(f"OK_RECENT(<=15m): {len(recent)} | OK_STALE(>15m): {len(stale)} | MISSING: {len(missing)} | SKIPLIST: {len(skip)}")
    ...
    # status CSV (one row per ticker in union order)
    status_map = debug_df.drop_duplicates(subset=['ticker'], keep='last').set_index('ticker')[['status','yahoo_ts','age_min']].to_dict('index') if not debug_df.empty else {}
    status_rows = []
    for t in union:
        meta = status_map.get(t, {})
        px, ts = got.get(t, (np.nan, ""))
        status_rows.append({
            'ticker': t,
            'premarket': px,
            'yahoo_ts': ts,
            'status': meta.get('status', 'missing' if t in missing else ('skipped' if (not args.refresh_bad and t in skip) else 'unknown')),
            'age_min': meta.get('age_min', np.nan),
        })
    status_df = pd.DataFrame(status_rows)
    status_df.to_csv('premarket_auto_status.csv', index=False)
