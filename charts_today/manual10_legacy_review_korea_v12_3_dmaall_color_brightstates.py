...
BLUE = "\033[1;94m"
RESET = "\033[0m"
BG_GREEN = "\033[30;102m"
BG_CYAN = "\033[30;106m"
BG_YELLOW = "\033[30;103m"
BG_RED = "\033[37;101m"
BG_MAGENTA = "\033[30;105m"
...
def color_section_title(title: str) -> str:
    if title.startswith("NEW ENTRY"):
        return f"{BG_GREEN}{title}{RESET}"
    if title.startswith("HOLD ONLY"):
        return f"{BG_YELLOW}{title}{RESET}"
    if title.startswith("AVOID NEW"):
        return f"{BG_RED}{title}{RESET}"
    return title


def color_entry_state(state: str) -> str:
    if state == "PREBREAK OK":
        return f"{BG_CYAN}{state}{RESET}"
    if state == "ENTRY OK":
        return f"{BG_GREEN}{state}{RESET}"
    if state == "SMALL SIZE":
        return f"{BG_YELLOW}{state}{RESET}"
    if state == "HOLD ONLY":
        return f"{MAGENTA}{state}{RESET}"
    if state == "AVOID NEW":
        return f"{BG_RED}{state}{RESET}"
    if state == "WATCH":
