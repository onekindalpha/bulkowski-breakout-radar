#!/usr/bin/env python3
"""
bulkowski_scan_from_debugcsv_strict_v5_ranked_korea.py
...
Usage:
  python bulkowski_scan_from_debugcsv_strict_v5_ranked_korea.py --top 10 --out candidates.txt
  python bulkowski_scan_from_debugcsv_strict_v5_ranked_korea.py --top 15 --max-2x 5 --out candidates.txt
"""
...
def volume_confirmed(df: pd.DataFrame, idx: int):
    """Return (confirmed_bool, vol_ratio_vs_avg20)."""
    vol = df["Volume"].astype(float)
    avg = vol.rolling(VOL_AVG).mean().shift(1)
    if pd.isna(avg.iloc[idx]) or avg.iloc[idx] == 0:
        return False, float('nan')
    ratio = float(vol.iloc[idx] / avg.iloc[idx])
    return bool(ratio >= VOL_MULT), ratio
...
def main():
    ...
    for sym in universe:
        ...
        if bidx is None:
            ...
            vol_ok, vol_ratio = volume_confirmed(df, bidx)
        ...
        rows.append({
            ...
            "vol_ratio": round(float(vol_ratio), 3) if vol_ratio==vol_ratio else np.nan,
            "hold_confirmed": bool(hold_ok),
            "breakout_pct": round(((last_close/blevel)-1)*100, 3) if (blevel and blevel==blevel and blevel>0) else np.nan,
    ...
    out = out.sort_values(["rank","hold_confirmed","vol_ratio","breakout_pct","is_2x","symbol"],
                        ascending=[True, False, False, False, True, True]).drop(columns=["rank"]).reset_index(drop=True)

    print("\n=== BULKOWSKI STRICT CANDIDATES (v5 ranked, includes 2x) ===")
