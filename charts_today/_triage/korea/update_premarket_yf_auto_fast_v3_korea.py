...
from pipeline_config_korea import load_default_groups, print_group_counts, union_ordered, now_kst_str
...
def yf_quote(yf, ticker: str):
    """
    KRX-friendly "snapshot" quote.
    Tries intraday bars first; falls back to last daily close.
    Returns ((price, ts), err)
    """
    # 1) intraday (no pre/post for KRX)
    try:
        df = yf.download(ticker, period="5d", interval="1m", prepost=False, progress=False)
        ...
        pass
    # 2) fallback intraday 5m
    try:
        df = yf.download(ticker, period="10d", interval="5m", prepost=False, progress=False)
        if df is not None and not df.empty and "Close" in df.columns:
            px = float(df["Close"].dropna().iloc[-1])
            ts = df.index[-1]
            return (px, ts), ""
    ...
    # 3) daily fallback
    try:
        df = yf.download(ticker, period="30d", interval="1d", auto_adjust=False, progress=False)
        if df is not None and not df.empty and "Close" in df.columns:
            px = float(df["Close"].dropna().iloc[-1])
            ts = df.index[-1]
            return (px, ts), ""
...
def main():
    ...
    pd.DataFrame(rows).to_csv("premarket_auto_korea.csv", index=False)
    pd.DataFrame(dbg).to_csv("premarket_auto_debug_korea.csv", index=False)
    print("Saved: premarket_auto_korea.csv")
    print("Saved: premarket_auto_debug_korea.csv")
