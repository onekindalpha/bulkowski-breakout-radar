...
def pick_group_table(tables: list[pd.DataFrame]) -> pd.DataFrame:
    for df in tables:
        cols = {str(c).strip() for c in df.columns}
        if "Name" in cols and "Change" in cols:
            return df.copy()
    raise RuntimeError("Could not find Finviz group table with Name/Change columns.")
...
def fetch_group_rankings(
    ...
    # v=152 exposes Name / Change / Stocks on the current public groups pages.
    # o=-change sorts descending by current-day Change.
    url = f"https://finviz.com/groups.ashx?g={kind}&v=152&o=-change"
    html = client.get(url)
    table = pick_group_table(read_html_tables(html))
    ...
    if sort_by == "change":
        ...
        # Keep current page order if unsupported sort key is passed.
        pass
...
def pick_screener_table(tables: list[pd.DataFrame]) -> pd.DataFrame:
    for df in tables:
        cols = [str(c).strip() for c in df.columns]
        cols_set = set(cols)
        if {"Ticker", "Company", "Sector", "Industry", "Price", "Change"}.issubset(cols_set):
            return df.copy()
    raise RuntimeError("Could not find Finviz screener constituents table.")
...
def fetch_group_constituents(
    ...
    while pages:
        ...
        html = client.get(url)
        df = pick_screener_table(read_html_tables(html))
...
def write_txt(
    ...
    lines.append(f"# source=finviz_groups")
...
def main() -> int:
    ...
    for g in selected_groups:
        print(f"[group] {g.kind} #{g.rank} {g.name}  change={'' if g.change is None else f'{g.change:.2f}%'}", file=sys.stderr)
    ...
    write_txt(out_path, tickers, selected_groups, members, len(existing_tickers) if existing_paths else None, append_mode=args.append)
