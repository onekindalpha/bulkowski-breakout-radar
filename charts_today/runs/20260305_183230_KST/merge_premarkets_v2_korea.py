...
AUTO = Path("premarket_auto_korea.csv")
MANUAL = Path("premarket_manual_korea.csv")
OUT = Path("premarket_korea.csv")
...
def main():
    ...
    if a.empty and m.empty:
        ...
        print("Saved premarket_korea.csv (0 tickers).")
        return
    ...
    print(f"Saved premarket_korea.csv ({len(combined)} tickers).")
    if not m.empty:
