#!/usr/bin/env python3
"""
watch_positions_notify_v4.py

Fixes the "sudden SELL_SIGNAL even when price is above break_level" issue.

Key idea: ARM the break-level failure logic only AFTER you have seen at least one DAILY CLOSE above break_level
while this watcher is running.

- WARN:
    A) drawdown <= -warn-dd (default 3%)
    B) current price < break_level (intraday)
- SELL_SIGNAL:
    A) drawdown <= -stop-dd (default 5%)
    B) (ARMED) daily CLOSE below break_level*(1-sell_buffer%) for N days (default 2)

State is persisted in .notify_state.json:
  - status (OK/WARN/SELL_SIGNAL)
  - last_ts
  - armed (True/False)  <-- new

Price source:
  - Primary: yfinance 1m prepost=True batch
  - Fallback: premarket.csv (optional)

Usage:
  python watch_positions_notify_v4.py --watch --interval 60 --sell-close-days 2 --sell-buffer 0.3
"""
    ...
    for pos in positions:
        ...
        if ddf is None or ddf.empty:
            rows.append({
                ...
                "px_source": px_src,
                "armed": state.get(t, {}).get("armed", False),
            })
            continue
        ...
        # arm logic: once we have a daily close ABOVE (or equal) break_level, arm it
        armed_prev = bool(state.get(t, {}).get("armed", False))
        armed_now = armed_prev or bool(last_close >= level)

        # support-based signals
        warn_support = bool(np.isfinite(cur_px) and (cur_px < level))
        sell_support = bool(armed_now and close_below_for_n_days(close, level, sell_close_days, sell_buffer))
        ...
        if sell_dd_hit or sell_support:
            status = "SELL_SIGNAL"
            reason = "dd<=stop" if sell_dd_hit else f"armed_close_below_break_{sell_close_days}d"
        elif warn_dd_hit or warn_support:
            status = "WARN"
            reason = "dd<=warn" if warn_dd_hit else ("px_below_break(armed)" if armed_now else "px_below_break(unarmed)")
...
def main():
    ...
    print(f"support: WARN=px<level  SELL=(ARMED) close<{args.sell_close_days}d below level*(1-{args.sell_buffer}%)")
    print(f"daily_refresh_min={args.daily_refresh_min}")
    print("Price source: yfinance 1m prepost=True (fallback: premarket.csv)")
    print("ARMING: break-level failure checks activate only after a daily close >= break_level is observed.\n")
    ...
    def run_once():
        ...
        cols = ["ticker","status","entry","current_px","drawdown_pct","break_level","armed","reason","px_source"]
