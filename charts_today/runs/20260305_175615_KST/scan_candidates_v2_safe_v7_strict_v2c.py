...
def load_candidates_meta(path: str = "candidates_meta.csv") -> dict[str, dict]:
...
def main():
    ...
    ap.add_argument('--break-mode', choices=['a','b'], default='b', help='A=use auto prior-60d-high break_level (ignore pattern override); B=use pattern break_level from candidates_meta.csv when available.')
    ...
    use_pattern_override = (break_mode == 'b')
...
cand_meta = load_candidates_meta("candidates_meta.csv")
if cand_meta:
    print(f"candidates_meta.csv: {len(cand_meta)} tickers (pattern break_level overrides enabled)")
else:
...
print(f"KST_NOW: {now_kst_str()}\n")
    ...
    header = [
        ...
        f"used_candidates_meta,{bool(cand_meta)}",
