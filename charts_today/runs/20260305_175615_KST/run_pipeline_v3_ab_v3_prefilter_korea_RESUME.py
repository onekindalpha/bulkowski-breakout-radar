...
def main():
    ...
    ap.add_argument("--from-step", choices=["start","manual","merge","scan2","report"], default="start",
                    help="Resume pipeline from a step: start=all, manual=manual entry, merge=merge+scan2+report, scan2=scan2+report, report=report only")
    args = ap.parse_args()
    ...
    do_sync = args.from_step == "start"
    do_prefilter = args.from_step == "start" and (not args.no_prefilter)
    do_update = args.from_step == "start"
    do_scan1 = args.from_step == "start"
