...
HEADERS = {
    "APCA-API-KEY-ID": os.getenv("APCA_API_KEY_ID", "PKA2GMWBYUJWQNBKLD5HJLJBUV"),
    "APCA-API-SECRET-KEY": os.getenv("APCA_API_SECRET_KEY", "GKsR4Th85nZgfP3K2EpDHbDPaVEAoNGR5GB9GrFiHXbR"),
...
def get_latest_trades(symbols):
    ...
    params = {"symbols": ",".join(symbols), "feed": "iex"}  # 무료는 IEX
...
def main():
    ...
    with open("premarket_auto.csv", "w", newline="", encoding="utf-8") as f:
        ...
        f.write(f"# saved_at_kst,{now_kst}\n")
    ...
    print(f"Saved {out_path} ({len(all_tickers)} tickers). Missing: {len(missing)}. saved_at_kst={now_kst}")
    if missing:
