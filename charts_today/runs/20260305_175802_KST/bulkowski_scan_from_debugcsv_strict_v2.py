...
Changes vs strict v1:
- Can include multiple groups from premarket_auto_debug.csv via --groups.
- Default groups: tickers_core, finviz_manual (so you can keep finviz_manual.txt separate)
- Writes candidates.txt (optional) as before.

Usage:
  python bulkowski_scan_from_debugcsv_strict_v2.py --debug premarket_auto_debug.csv --top 10 --out candidates.txt
  python bulkowski_scan_from_debugcsv_strict_v2.py --groups tickers_core,finviz_manual --top 15 --out candidates.txt
"""
...
def load_universe(debug_csv: str, groups: set[str]) -> list[str]:
    ...
    grp_col = "group" if "group" in df.columns else None
    if grp_col:
        df = df[df[grp_col].astype(str).isin(groups)]
    df = df[ok_err].copy()

    tickers = df[sym_col].dropna().astype(str).str.upper().unique().tolist()

    # sanity + exclude 2x by default (you can change this if you want)
...
def main():
    ...
    ap.add_argument("--groups", default="tickers_core,finviz_manual",
                    help="comma-separated groups in debug csv to include (default: tickers_core,finviz_manual)")
    args = ap.parse_args()
    ...
    print("\n=== BULKOWSKI STRICT CANDIDATES (v2) ===")
