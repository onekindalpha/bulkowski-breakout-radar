...
# --- KRX PDF: Hangul font fallback (mac/windows/linux) ---
...
#!/usr/bin/env python3
"""
make_buy_report_korea_v7.py
- CSV always
- XLSX when --xlsx (colored)
- PDF when --pdf (matplotlib table; no reportlab)
...
Writes buy_report_korea_latest.txt with output filenames.
"""
...
def print_terminal_blocks(out_df, *, title_prefix=""):
    """Print BUY_NOW / NEAR_BUY / TOP PRIORITY blocks to terminal, and return the text."""
    lines = []
    ts = datetime.now(KST).strftime("%Y-%m-%d %H:%M:%S")
    hdr = f"{title_prefix}BUY OPINION (KST {ts})".strip()
    lines.append(hdr)
    lines.append("")

    buy = out_df[out_df.get("BUY_NOW","N") == "Y"].copy()
    near = out_df[(out_df.get("near_buy", False) == True) & (out_df.get("BUY_NOW","N") != "Y")].copy()
    top = out_df.sort_values(["PRIORITY_RANK"], ascending=True).head(15).copy() if "PRIORITY_RANK" in out_df.columns else out_df.head(15).copy()

    lines.append("=== BUY_NOW (Y) ===")
    if len(buy) == 0:
        lines.append("(none)")
    else:
        for _, r in buy.iterrows():
            lines.append(_fmt_row(r, ["PRIORITY_RANK","ticker","price","daily_break_level","breakout_pct_vs_level","PRIORITY_SCORE","BUY_NOW_REASON"]))

    lines.append("")
    lines.append("=== NEAR_BUY (CloseBreak=Y & VolConfirm=Y, but BUY_NOW=N) ===")
    if len(near) == 0:
        lines.append("(none)")
    else:
        for _, r in near.sort_values(["PRIORITY_RANK"], ascending=True).head(15).iterrows():
            lines.append(_fmt_row(r, ["PRIORITY_RANK","ticker","price","daily_break_level","breakout_pct_vs_level","PRIORITY_SCORE","BUY_NOW_REASON"]))

    lines.append("")
    lines.append("=== TOP PRIORITY (ranked) ===")
    for _, r in top.iterrows():
        lines.append(_fmt_row(r, ["PRIORITY_RANK","BUY_NOW","ticker","price","daily_break_level","breakout_pct_vs_level","room_to_weekly_r1_pct","rsi14","chase_flag","near_buy","BUY_NOW_REASON"]))
...
def newest_report_path() -> Path:
    stamped = sorted(Path(".").glob("report_v2_korea_*_KST.csv"), key=lambda p: p.stat().st_mtime, reverse=True)
    if stamped:
        return stamped[0]
    p = Path("report_v2_korea.csv")
    if not p.exists():
        raise FileNotFoundError("No report found: report_v2_korea.csv or report_v2_korea_*_KST.csv")
    return p
...
def main():
    ...
    out["near_buy"] = out.apply(lambda r: (bool(r.get("breakout_confirmed_close")) and bool(r.get("breakout_volume_confirmed")) and r.get("BUY_NOW")=="N"), axis=1)
    ...
    csv_name = f"buy_report_korea_{ts}_KST.csv"
    out.to_csv(csv_name, index=False)

    # Terminal summary
    print(f"Source: {rp.name}")
    print(f"Saved: {csv_name} (rows={len(out)} | BUY_NOW=Y: {int((out['BUY_NOW']=='Y').sum())})")
    print("\n=== BUY_NOW (Y) ===")
    y = out[out["BUY_NOW"]=="Y"]
    if y.empty:
        print("(none)")
    else:
        cols_y = [c for c in ["PRIORITY_RANK","ticker","price","daily_break_level","breakout_pct_vs_level","PRIORITY_SCORE","BUY_NOW_REASON"] if c in out.columns]
        print(y[cols_y].to_string(index=False))

    print("\n=== NEAR_BUY (CloseBreak=Y & VolConfirm=Y, but BUY_NOW=N) ===")
    nb = out[(out["near_buy"]==True) & (out["BUY_NOW"]=="N")]
    if nb.empty:
        print("(none)")
    else:
        cols_nb = [c for c in ["PRIORITY_RANK","ticker","price","daily_break_level","breakout_pct_vs_level","PRIORITY_SCORE","chase_flag","BUY_NOW_REASON"] if c in out.columns]
        print(nb[cols_nb].head(10).to_string(index=False))

    print("\n=== TOP PRIORITY (ranked) ===")
    cols_top = [c for c in ["PRIORITY_RANK","ticker","BUY_NOW","PRIORITY_SCORE","breakout_pct_vs_level","room_to_weekly_r1_pct","rsi14","chase_flag","near_buy","BUY_NOW_REASON"] if c in out.columns]
    print(out[cols_top].head(15).to_string(index=False))
    ...
    if args.xlsx:
        try:
            ...
            xlsx_name = f"buy_report_korea_{ts}_KST.xlsx"
            wb = Workbook()
            ws = wb.active
            ws.title = "buy_report"
            ...
            buy_col = header.index("BUY_NOW") + 1
    ...
    if args.pdf:
        try:
            ...
            pdf_name = f"buy_report_korea_{ts}_KST.pdf"
            # SUMMARY PAGE
            buy_rules = (
                f"BUY_NOW: CloseBreak=Y & VolConfirm=Y"
                + ("" if args.no_intraday else f" & IntradayHold=Y(hold)")
                + f", Chase<={args.buy_chase_pct:.1f}%, RoomR1>={args.buy_min_room_pct:.1f}%, RSI[{args.buy_rsi_min:.0f},{args.buy_rsi_max:.0f}]"
            )
            ...
            with PdfPages(pdf_name) as pdf:
                ...
                ax.axis("off")
                title = f"Buy Report SUMMARY (KST {datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')})  Source={rp.name}"
                ax.set_title(title, fontsize=12, pad=10)
                y = out[out["BUY_NOW"]=="Y"]
                nb = out[(out.get("near_buy", False)==True) & (out["BUY_NOW"]=="N")]
                lines = [buy_rules, "", f"BUY_NOW(Y) count: {len(y)}"]
                if len(y) == 0:
                    lines.append("  - (none)")
                else:
                    for _, r in y.head(20).iterrows():
                        lines.append(f"  - {r.get('ticker')}  px={r.get('price')}  lvl={r.get('daily_break_level')}  reason={r.get('BUY_NOW_REASON')}")
                lines += ["", f"NEAR_BUY count: {len(nb)}"]
                if len(nb) == 0:
                    lines.append("  - (none)")
                else:
                    for _, r in nb.head(20).iterrows():
                        lines.append(f"  - {r.get('ticker')}  px={r.get('price')}  lvl={r.get('daily_break_level')}  fail={r.get('BUY_NOW_REASON')}")
    ...
    # pointer file
    try:
        latest_txt = Path("buy_report_korea_latest.txt")
        lines = [f"CSV={csv_name}"]
        if xlsx_name:
            lines.append(f"XLSX={xlsx_name}")
        if pdf_name:
