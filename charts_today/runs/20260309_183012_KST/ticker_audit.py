...
from pipeline_config import load_default_groups, union_ordered, print_group_counts
...
def main():
    ...
    auto_dbg = load_csv_tickers("premarket_auto_debug.csv", ["ticker","yahoo_symbol"])
    cands = load_txt_tickers("candidates.txt")
    pm_manual = load_csv_tickers("premarket_manual.csv", ["ticker"])
    pm = load_csv_tickers("premarket.csv", ["ticker"])
    report = load_csv_tickers("report_v2.csv", ["ticker"])
