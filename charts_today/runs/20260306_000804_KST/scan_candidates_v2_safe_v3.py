...
def build_universe(universe_file: str | None, premarket: dict) -> list[str]:
    """
    핵심 수정:
      - 후보 10개(LNG 등)를 manual로 넣어도, scan은 tickers.txt만 돌면 결과에 안 나옴.
      - 그래서 premarket.csv에 있는 티커는 무조건 universe에 합침.
    우선순위:
      1) CLI --universe
      2) candidates.txt (있으면)
      3) tickers.txt
    """
    if universe_file:
        base = load_tickers(universe_file)
    elif Path("candidates.txt").exists():
        base = load_tickers("candidates.txt")
    else:
        base = load_tickers("tickers.txt")

    # premarket 티커를 universe에 합치기 (manual로만 들어온 애들도 포함)
    seen = set()
    out = []
    for t in base:
        if t not in seen:
...
def main():
    ...
    ap.add_argument("--universe", default=None, help="tickers file (e.g. candidates.txt). default: candidates.txt if exists else tickers.txt")
    ap.add_argument("--premarket", default="premarket.csv", help="merged premarket csv (default: premarket.csv)")
    args = ap.parse_args()

    premarket = load_premarket(args.premarket)
    tickers = build_universe(args.universe, premarket)
