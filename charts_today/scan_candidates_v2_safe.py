...
def breakout_and_retest(df: pd.DataFrame, price: float, lookback=60, tol_pct=0.6):
    """Legacy-friendly breakout level.

    break_level is defined as: max(High) of the *previous* N sessions (exclude today).
    This matches "옵션 A = 전일까지 60일 고점 돌파".
    """
    hi = float(df["High"].shift(1).rolling(lookback).max().iloc[-1])
    breakout = price > hi
    retest = (abs(price - hi) / hi * 100) <= tol_pct
    return hi, breakout, retest


def breakout_meta_option_a(df: pd.DataFrame, lookback=60, vol_mult=1.2):
    """Compute breakout_date / vol_confirmed / hold_confirmed for Option-A (prior N-day high).

    - breakout_date: most recent session where Close crosses above prior-N-day-high.
    - break_level_at_breakout: the prior-N-day-high value on breakout_date.
    - vol_confirmed: breakout day Volume >= vol_mult * avg(Volume,20) of prior sessions.
    - hold_confirmed: since breakout_date, Close has not closed back below break_level_at_breakout
                      and there are at least 2 closes above it.

    Notes:
    - This is intentionally simple/transparent for automation; it doesn't try to infer patterns.
    """
    if df is None or df.empty or len(df) < (lookback + 5):
        return None, None, False, False

    close = df["Close"].astype(float)
    hi_prev = df["High"].shift(1).rolling(lookback).max()
    breakout_mask = close > hi_prev
    prev = breakout_mask.shift(1).fillna(False)
    start = breakout_mask & (~prev)
    if not bool(start.any()):
        return None, None, False, False

    bd = start[start].index[-1]
    lvl = float(hi_prev.loc[bd])

    # volume confirm on breakout day
    vol = df["Volume"].astype(float)
    vol20 = vol.shift(1).rolling(20).mean()
    v_ok = bool(vol.loc[bd] >= (vol_mult * float(vol20.loc[bd])) if pd.notna(vol20.loc[bd]) else False)

    # hold confirm
    after = close.loc[bd:]
    above = (after >= lvl)
    hold_ok = bool(above.all() and above.sum() >= 2)

    return pd.to_datetime(bd).date().isoformat(), lvl, v_ok, hold_ok
...
def main():
    ...
    for t in tickers:
        try:
            ...
            # Option-A breakout meta (prior-60d high breakout)
            breakout_date, break_level_at_breakout, vol_confirmed, hold_confirmed = breakout_meta_option_a(df, lookback=60, vol_mult=1.2)
            ...
            # Use the breakout-day level when available (better anchor for "얼마나 넘었는지")
            anchor_break_level = float(break_level_at_breakout) if break_level_at_breakout is not None else float(daily_break_level)

            # confirm flags expected by buy_report
            breakout_confirmed_close = (last_close > anchor_break_level) if anchor_break_level else False
            breakout_volume_confirmed = bool(vol_confirmed)
            breakout_confirmed_intraday = False  # not derivable from daily OHLCV safely
            ...
            row = {
                ...
                # legacy / C-output style fields
                "break_level": float(anchor_break_level),
                "breakout_date": breakout_date,
                "vol_confirmed": bool(vol_confirmed),
                "hold_confirmed": bool(hold_confirmed),
                # buy-report compat flags
    ...
    for c in ["price", "last_close", "gap_pct", "rsi14", "px_vs_sma50", "px_vs_sma200",
              "room_to_weekly_r1_pct", "weekly_r1", "weekly_s1", "daily_break_level", "break_level", "score"]:
    ...
    cols = [
        ...
        "break_level", "breakout_date", "vol_confirmed", "hold_confirmed",
