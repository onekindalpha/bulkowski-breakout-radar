...
def color_label(label: str) -> str:
    ...
    if label == "NEAR BREAKOUT":
        return f"\033[30;106m{label}{RESET}"
...
def color_section_title(title: str) -> str:
    if title.startswith("NEW ENTRY"):
        return f"\033[30;102m{title}{RESET}"
...
def color_entry_state(state: str) -> str:
    if state == "PREBREAK OK":
        return f"\033[30;106m{state}{RESET}"
    if state == "ENTRY OK":
        return f"\033[30;102m{state}{RESET}"
    if state == "SMALL SIZE":
        return f"\033[30;103m{state}{RESET}"
    if state == "HOLD ONLY":
