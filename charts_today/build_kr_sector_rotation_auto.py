...
# -*- coding: utf-8 -*-
"""
build_kr_sector_rotation_auto.py

B = 업종/섹터 로테이션 참고용 overlay
- seed 한국 티커들에 대해 yfinance info의 sector/industry를 조회
- 1일/1주/1달 종목 성과를 바탕으로 섹터 점수 계산
- 강한 섹터 상위 N개 안에서 리더 종목들을 뽑아
- kr_sector_rotation_auto.txt 로 저장

Outputs:
- kr_sector_rotation_auto.txt
- kr_sector_rotation_groups.csv
- kr_sector_rotation_members.csv

주의:
- Yahoo sector/industry 분류는 완벽하지 않을 수 있음
- 그래도 “업종 참고용 B 레이어”로는 실전적으로 쓸 만한 구조
"""
...
def fetch_sector_info(sym: str) -> tuple[str, str]:
    try:
        tk = yf.Ticker(sym)
        info = {}
        # get_info can fail; use .info lazily
...
def fetch_rows(tickers: list[str]) -> pd.DataFrame:
    rows = []
    total = len(tickers)
    for i, sym in enumerate(tickers, 1):
        try:
            df = yf.download(sym, period="3mo", interval="1d", progress=False, auto_adjust=False, group_by="column")
...
def main():
    ...
    df = fetch_rows(tickers)
    if df.empty:
        Path("kr_sector_rotation_auto.txt").write_text("", encoding="utf-8")
        pd.DataFrame(columns=["sector","score","change","perf_week","perf_month"]).to_csv("kr_sector_rotation_groups.csv", index=False)
        pd.DataFrame(columns=["sector","ticker","industry","change","perf_week","perf_month","leader_score"]).to_csv("kr_sector_rotation_members.csv", index=False)
        raise SystemExit("Could not fetch any yfinance rows for Korea sector-rotation overlay.")

    # Per-stock leader score
    ...
    # Sector score = mean of top 3 leaders in each sector (or fewer if needed)
    grp_rows = []
