...
def classify_row(r: pd.Series, near_max_dist: float, chase_ext_max: float):
    ...
    if label in {"BUY NOW", "NEAR BREAKOUT"}:
        if overbought or low_score:
            entry_state = "AVOID NEW"
            if overbought and low_score:
...
def sort_key(ticker: str, row: pd.Series, info: dict):
    ...
    # v11 preference: after entry state / label / retest / asset,
    # promote higher-quality setups (grade) before room/dist micro-ordering.
    return (
...
def print_summary(title: str, items, allowed_states: set[str], topn: int):
    ...
    for i, (t, row, info) in enumerate(picks[:topn], start=1):
        ...
        print(
            f"{i:02d}. {t} {name} [{asset}] | entry={info['entry_state']} | {info['label']} | "
...
def print_focus_details(title: str, items, safe_text: str, detail_top: int):
    print(f"\n--- {title} ---")
    picks = [(t, row, info) for t, row, info in items if info["entry_state"] in {"ENTRY OK", "SMALL SIZE", "HOLD ONLY", "AVOID NEW", "WATCH"}]
    if not picks:
        print("(none)")
        return
...
def main():
    ...
    print("=== V11 ENTRY-STATE REVIEW ===")
    print(f"manual tickers in report: {len(manual_set & safe_set)} | safe top: {len(safe_set)}")
    print("Ranking priority: ENTRY OK > SMALL SIZE > HOLD ONLY > AVOID NEW > WATCH > REJECT")
    print("Within each state: BUY NOW > NEAR BREAKOUT > retest > STOCK > ETF > grade(A/B/C) > wider room > closer to break > less extended")
    print("Default view = actionable summaries first. HOLD ONLY / AVOID NEW are separated so they stop polluting new-entry picks.")

    print_summary("NEW ENTRY / MANUAL ∩ SAFE", inter, {"ENTRY OK", "SMALL SIZE"}, args.quick_top)
    print_summary("NEW ENTRY / SAFE ONLY", sonly, {"ENTRY OK", "SMALL SIZE"}, args.quick_top)
    ...
    print("\n(기본은 actionable summary + 각 풀 상위 상세만 보여줌. 바쁠 때 신규매수 가능/홀드전용/신규비추를 바로 구분하려는 목적)")
    ...
    print("- AVOID NEW = 과열 / 점수 문제 등으로 신규 진입 비추")
    print("- --detail-top 3 처럼 늘리면 상세를 더 볼 수 있음")
    print("- --full-details 붙이면 그룹별 전체 상세를 다 봄")
