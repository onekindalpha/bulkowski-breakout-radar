...
def normalize_screener_url(url: str, view: int = 111) -> str:
    """Force Finviz screener links into a stable overview view with expected columns."""
    p = urlparse(url)
...
def select_best_constituent_columns(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df.columns = [str(c).strip() for c in df.columns]
    required_core = ["Ticker", "Company", "Price", "Change"]
    missing_core = [c for c in required_core if c not in df.columns]
    if missing_core:
        raise RuntimeError(f"Missing screener columns: {missing_core}; got={list(df.columns)}")

    for opt in ["Sector", "Industry"]:
        if opt not in df.columns:
            df[opt] = ""

    return df[["Ticker", "Company", "Sector", "Industry", "Price", "Change"]].copy()
...
def fetch_group_constituents(
    ...
    pages: list[str] = [normalize_screener_url(group.url, 111)]
    seen_pages: set[str] = set()
    frames: list[pd.DataFrame] = []

    required = {"Ticker", "Company", "Price", "Change"}
    ...
    while pages:
        ...
        use = select_best_constituent_columns(df)
        ...
        for next_url in extract_pagination_links(html):
            next_url = normalize_screener_url(next_url, 111)
...
def main() -> int:
    ...
    ap.add_argument("--out", type=str, default="finviz_top_groups_auto_mixed.txt", help="output txt path")
    ap.add_argument("--groups-csv", type=str, default="finviz_top_groups_mixed_groups.csv", help="save selected groups here")
    ap.add_argument("--members-csv", type=str, default="finviz_top_groups_mixed_members.csv", help="save constituent rows here")
