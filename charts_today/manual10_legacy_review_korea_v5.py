...
def classify_row(r: pd.Series, near_max_dist: float, chase_ext_max: float):
    ...
    # Sell / exit hint
    if weekly_r1 is not None and room_left is not None:
        if room_left > 0:
            sell_hint = f"1차 익절 후보: weekly_r1 {weekly_r1:.2f} (현재가 대비 남은 공간 {room_left:.2f})"
        else:
            sell_hint = f"weekly_r1 {weekly_r1:.2f} already passed; break_level {brk:.2f} 재이탈 시 정리"
    elif brk:
        if breakout:
            sell_hint = f"break_level {brk:.2f} 재이탈 시 정리"
        else:
            sell_hint = f"break_level {brk:.2f} 돌파 후 반응 확인"
    else:
        sell_hint = "exit hint unavailable"
...
def print_row(ticker: str, info: dict, safe_text: str):
    ...
    print(f"    sell_hint: {info['sell_hint']}")
    if info["warnings"]:
...
def main():
    ...
    print("- sell_hint = practical first take-profit / stop management clue")
