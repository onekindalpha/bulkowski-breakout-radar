...
# -*- coding: utf-8 -*-
"""
build_kr_strong_stocks_auto_v2.py

A = 강한 종목 리더용 overlay
- seed 파일들에서 한국 티커를 읽고
- yfinance 1일/1주/1달 혼합 점수로 리더 종목을 뽑아
- kr_strong_stocks_auto.txt 로 저장

Outputs:
- kr_strong_stocks_auto.txt
- kr_strong_stocks_auto.csv
"""
...
def main():
    ...
    if df.empty:
        Path("kr_strong_stocks_auto.txt").write_text("", encoding="utf-8")
        pd.DataFrame(columns=["ticker","score","change","perf_week","perf_month"]).to_csv("kr_strong_stocks_auto.csv", index=False)
        raise SystemExit("Could not fetch any yfinance momentum rows for Korea strong-stocks overlay.")
