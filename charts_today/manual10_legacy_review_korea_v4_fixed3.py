...
def robust_read_csv(path: str) -> pd.DataFrame:
    # Your manual/report files are standard CSV with optional leading comment lines.
...
def classify_row(r: pd.Series, safe_flag: bool, room_min: float, near_max_dist: float, chase_ext_max: float):
    ...
    if (
        ...
        label = "BUY NOW"
        why = "manual+safe confirmed; breakout and retest confirmed; room sufficient"
    else:
        if breakout and retest and sma50_ok and sma200_ok:
            if room is not None and room < room_min:
                label = "WATCH"
                why = "breakout confirmed, but weekly room too small"
            elif not safe_flag:
                label = "WATCH"
                why = "breakout confirmed, but not in SAFE intersection"
            elif ext is not None and ext > chase_ext_max:
                label = "WATCH / CHASE"
                why = f"already extended ({ext:+.2f}%)"
            else:
                label = "WATCH"
                why = "breakout confirmed, but strict BUY NOW conditions not fully met"
...
def print_row(ticker: str, info: dict, safe_text: str):
    room_text = "" if info["room"] is None else f"{info['room']:.2f}%"
    print(
        ...
        f"rsi={fmt_num(info['rsi'])} | room={room_text} | "
...
def main():
    ...
    if not inter:
        ...
        for t in inter:
            info = classify_row(report_by_ticker[t], True, args.room_min, args.near_max_dist, args.chase_ext_max)
            print_row(t, info, "Y")
