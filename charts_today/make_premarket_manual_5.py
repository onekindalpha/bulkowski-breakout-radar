#!/usr/bin/env python3
"""
make_premarket_manual_5.py (replacement)
...
Usage
  python make_premarket_manual_5.py
  python make_premarket_manual_5.py --max 10
"""
...
from pipeline_config import load_default_groups, union_ordered, now_kst_str, read_tickers_from_file, write_header_lines
...
def main():
    ...
    ap.add_argument("--file", "-f", type=str, default="candidates.txt",
                    help="path to candidates file (default: candidates.txt)")
    args = ap.parse_args()
    ...
    if candidates_path.exists():
        ...
        for i, t in enumerate(cands, 1):
            px = None
            while px is None:
                px = prompt_float(f"[{i}/{len(cands)}] {t} Price: ")
                # allow skip
                if px is None:
