...
# -*- coding: utf-8 -*-
"""
build_kr_foreign_naver_auto_v5.py

Improvements over v4:
- Broader date-context extraction (table text + parent/sibling context)
- Latest-date preference when available
- If score/rows tie and date is unavailable, prefer later block_idx/page order
  because Naver often renders newer date column on the right later in DOM.
- Writes date_label/context into debug csv.
"""
...
def context_with_dates(table) -> str:
    parts = []
    # self text first
    parts.append(clean_text(table.get_text(' ', strip=True)))
    # previous siblings nearby often contain date headers
    sib = table.previous_sibling
    count = 0
    while sib is not None and count < 5:
        txt = clean_text(getattr(sib, 'get_text', lambda *a, **k: str(sib))(' ', strip=True) if hasattr(sib, 'get_text') else str(sib))
        if txt:
            parts.append(txt)
        sib = sib.previous_sibling
        count += 1
    # parents a few levels up
    node = table
    for _ in range(4):
        node = node.parent
        if node is None:
...
def collect_candidate_blocks(html: str, page_url: str):
    ...
    for i, table in enumerate(soup.find_all("table")):
        ...
        ctx = context_with_dates(table)
        date_label = parse_date_label(ctx)
...
def pick_best_rows(blocks):
    ...
    # Later block_idx/page order wins tie because right/latest column often appears later in DOM.
    candidates = sorted(candidates, key=lambda b: (b["score"], len(b["rows"]), b.get("block_idx", -1)), reverse=True)
    best = candidates[0]
...
def main():
    ...
    if not rows:
        raise SystemExit('No candidate rows parsed from Naver foreign ranking.')
    ...
    for r in rows:
        code = r['code']
        suffix = suffix_map.get(code) or yfinance_probe_suffix(code, probe_cache)
        if not suffix:
            continue
        sym = code + suffix
        out.append({
            ...
            'amount': r.get('amount'),
            'qty': r.get('qty'),
            'source': 'naver_foreign_buy',
            'page_url': r.get('page_url', ''),
        })

    if not out:
        raise SystemExit('Rows parsed but no symbols could be mapped to .KS/.KQ.')

    df = pd.DataFrame(out)
    df = df.drop_duplicates(subset=['symbol']).copy()
    if 'amount' in df.columns:
        df['_sort'] = pd.to_numeric(df['amount'], errors='coerce').fillna(-1)
        df = df.sort_values(['_sort', 'symbol'], ascending=[False, True]).drop(columns=['_sort'])
    df = df.head(args.top)
...
', encoding='utf-8')
    ...
    print(f"Saved: {OUT_TXT} ({len(df)} tickers)")
    print(f"Saved: {OUT_CSV} ({len(df)} rows)")
    print(f"Saved: {DBG_CSV}")
    print('Top mapped names:')
    for _, row in df.head(10).iterrows():
        print(f" - {row['symbol']}  {row['name_kr']}  amount={row['amount']}")
