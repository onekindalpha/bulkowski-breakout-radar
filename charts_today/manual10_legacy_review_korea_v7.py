...
def classify_row(r: pd.Series, near_max_dist: float, chase_ext_max: float):
    ...
    if target_passed:
        manage = "target1 passed -> use break hold / recalc next target"
    elif room_small:
...
def sort_key(ticker: str, row: pd.Series, info: dict):
    ...
    target_penalty = 1 if info["target_passed"] else 0
...
def print_grouped(title: str, items, safe_text: str, show_rejects: bool):
    ...
    for t, row, info in items:
        cat = category_name(t, row, info)
        by_cat.setdefault(cat, []).append((t, row, info))
...
def print_quick_picks(title: str, items, topn: int):
    ...
    for i, (t, row, info) in enumerate(picks[:topn], start=1):
        ...
        print(
            ...
            f"dist={fmt_pct(info['dist'])} ext={fmt_pct(info['ext'])} rsi={fmt_num(info['rsi'])} | {info['manage']}"
        )
...
def main():
    ...
    print("=== V7 PRIORITIZED REVIEW ===")
    print(f"manual tickers in report: {len(manual_set & safe_set)} | safe top: {len(safe_set)}")
    print("Ranking priority: BUY NOW > NEAR BREAKOUT > STOCK > ETF > retest > closer to break > less extended")
    print("MANUAL ∩ SAFE is kept as the highest-priority pool. Within each pool, candidates are ranked.")
    print("BUY NOW still allows target1 already passed; in that case manage_line is the break and target must be recalculated.")
    ...
    print("- QUICK PICKS = only BUY NOW / NEAR BREAKOUT, already ranked for faster judgment")
    print("- manage = practical note for immediate decision / position handling")
    print("- Use --show-rejects if you still want the full reject dump")
