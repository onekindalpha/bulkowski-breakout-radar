...
"""
    ...
    # ══════════════════════════════════════════════
        ...
        if nhnl_df is not None and not nhnl_df.empty:
            ...
            # ── 이번 주 NH-NL 예상치 (주중 진행 데이터 기반) ──────────
            # df: 일별 breadth 데이터 (advances, declines, ad_diff)
            # 이번 주 월~오늘까지 데이터로 금요일 NH-NL 예상
            try:
                _df_daily = df.copy()
                _df_daily["dt"] = pd.to_datetime(_df_daily["date"].astype(str), format="%Y%m%d")
                # 이번 주 월요일 ~ 오늘
                _today = pd.Timestamp(datetime.today().date())
                _this_mon = _today - pd.Timedelta(days=_today.weekday())  # 이번 주 월요일
                _this_fri = _this_mon + pd.Timedelta(days=4)              # 이번 주 금요일
                _this_week = _df_daily[(_df_daily["dt"] >= _this_mon) & (_df_daily["dt"] <= _today)]

                if len(_this_week) >= 1 and _today <= _this_fri:
                    # 이번 주 진행된 일수
                    _days_done = len(_this_week)
                    _days_total = 5  # 월~금

                    # NH-NL 주간 집계: new_highs - new_lows 컬럼이 있으면 사용, 없으면 ad_diff 대리
                    if "new_highs" in _this_week.columns and "new_lows" in _this_week.columns:
                        _weekly_nh = int(_this_week["new_highs"].sum())
                        _weekly_nl = int(_this_week["new_lows"].sum())
                        _current_nhnl = _weekly_nh - _weekly_nl
                    else:
                        # ad_diff 합산으로 대리 계산 (실제 NH-NL과 스케일 다름 — 참고용)
                        _current_nhnl = int(_this_week["ad_diff"].sum())

                    # 금요일 예상: 현재 일평균 × 5일
                    _daily_avg = _current_nhnl / _days_done
                    _est_nhnl  = int(_daily_avg * _days_total)

                    # 마지막 확정 주 값
                    _last_confirmed_dt   = pf3["dt"].iloc[-1]
                    _last_confirmed_nhnl = float(pf3["nhnl"].iloc[-1])

                    # 점선: 마지막 확정값 → 이번 주 금요일 예상값
                    _est_label = (f"이번 주 예상 ({_days_done}일/{_days_total}일 기준)<br>"
                                  f"NH-NL 예상: {_est_nhnl:+,}")
                    fig_hl.add_trace(go.Scatter(
                        x=[_last_confirmed_dt, _this_fri],
                        y=[_last_confirmed_nhnl, _est_nhnl],
                        mode="lines+markers",
                        line=dict(color="rgba(255,220,100,0.8)", width=2, dash="dash"),
                        marker=dict(size=[0, 8], color="rgba(255,220,100,0.9)", symbol="circle-open"),
                        name=f"이번 주 예상 ({_days_done}/5일)",
                        hovertemplate=f"{_est_label}<extra></extra>",
                        xaxis="x", yaxis="y2",
                    ))
                    # 음영: 불확실 범위 (±20% 범위)
                    _margin = abs(_est_nhnl - _last_confirmed_nhnl) * 0.3 + 10
                    fig_hl.add_trace(go.Scatter(
                        x=[_last_confirmed_dt, _this_fri, _this_fri, _last_confirmed_dt],
                        y=[_last_confirmed_nhnl + _margin, _est_nhnl + _margin,
                           _est_nhnl - _margin, _last_confirmed_nhnl - _margin],
                        fill="toself",
                        fillcolor="rgba(255,220,100,0.08)",
                        line=dict(color="rgba(0,0,0,0)"),
                        showlegend=False, hoverinfo="skip",
                        xaxis="x", yaxis="y2",
                    ))
            except Exception:
                pass  # 예상치 계산 실패 시 조용히 넘김
